...
import { createAdvice } from '../../../../services/communityServices'
...
function AddAdviceModal({ isOpen, onClose, pais, ciudad, loading, setLoading, setWinKylets }) {
  const { logout, texts } = useUser();
  const screenTexts = texts.components.Blocks.Community.Modals.AddAdviceModal;
...
function handleCreateAdvice({ title, description, category, pais, ciudad, selectedMedia, logout, onSuccess, onError }) {
  if(!loading){
    setLoading(true)
    const formValues = {
      titulo: title,
    ...
    createAdvice(formValues, selectedMedia, logout)
      .then((response) => {
