...
import { View, Text, TouchableOpacity, ScrollView, Alert, StyleSheet, Modal, 
  KeyboardAvoidingView, Platform, Image, TextInput
} from 'react-native';
import { useUser } from "../../../../context/useUser";
import { createContacto } from '../../../../services/experienceServices'
import { SafeAreaView } from 'react-native-safe-area-context';
import GradientButton from '../../../Utils/GradientButton';
import { LinearGradient } from 'expo-linear-gradient';
import SearcherInput from '../../../Utils/SearcherInput';
...
function AddContactoModal({ isOpen, onClose, llamada, _id, type, loading, setLoading }) {
  const { logout, texts }=useUser()
  const screenTexts = texts.components.Wallet.Experiences.Modals.AddContactoModal
  ...
  const handleCreateExperience = async ({partId, puntuacion, user, preferPhone, preferEmail, description, category, selectedMedia, logout, onSuccess, onError }) => {
    if(!loading){
      setLoading(true)
      const formValues = {
        partId,
      ...
      try {  
        createContacto(formValues, selectedMedia, logout)
          .then(() => {
            llamada()
            onSuccess()
            setLoading(false)
  ...
  const handleSubmit = () => {
    if (!userInfo._id.trim()) {
      Alert.alert(screenTexts.ContactErrorMessages);
      return;
    }
    if (!description.trim()) {
    ...
    if (puntuacion === 0) {
      Alert.alert(screenTexts.RatingErrorMessages);
      return;
    }
    if(type === 'edit'){
    ...
    return (
      <View style={styles.stepContainer}>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.row}>
              <TouchableOpacity
                ...
                onPress={() => setPreferPhone(!preferPhone)}
              >
                <View style={[styles.checkbox, preferPhone && styles.checkboxChecked]} />
                <Text style={styles.checkboxLabel}>{screenTexts.CheckBox1Render2}</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={styles.checkboxContainer}
                onPress={() => setPreferEmail(!preferEmail)}
              >
                <View style={[styles.checkbox, preferEmail && styles.checkboxChecked]} />
                <Text style={styles.checkboxLabel}>{screenTexts.CheckBox2Render2}</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <StarRating onChangeRating={setPuntuacion}/>
        </View>
      </View>
    );
  };
...
export default AddContactoModal;