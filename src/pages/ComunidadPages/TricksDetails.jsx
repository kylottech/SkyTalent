...
import { infoTrick } from '../../services/communityServices'
...
const TricksDetails = ({route}) => {
  const navigate = useNavigation();
  const { isLogged, isLoading, texts, logout } = useUser();
  const screenTexts = texts.pages.ComunidadPages.TricksDetails;
  ...
  const handleInfoTrick = async () => {
    setLoading(true)
    setInfo([])
    try {
      await infoTrick( {paisId: paisId, ciudadId: ciudadId, categoriaId: categoriaId }, logout )
        .then((res) => {
          setInfo(res)
          setLoading(false)
        })
