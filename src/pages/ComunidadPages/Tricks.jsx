...
import { tagsTrick } from '../../services/communityServices'
import Top from "../../components/Utils/Top";
import TrickBlock from "../../components/Blocks/Community/TrickBlock";

const Tricks = ({route}) => {
  const navigate = useNavigation();
  const { isLogged, isLoading, texts, logout, translateTag, language } = useUser();
  const screenTexts = texts.pages.ComunidadPages.Tricks;
  const { paisId, ciudadId } = route.params
  const [info, setInfo] = useState([]);
  const [refreshing, setRefreshing] = useState(false);
  const [loading, setLoading] = useState(false);

  const handleTagsTrick = async () => {
    setLoading(true)
    setInfo([])
    try {
      await tagsTrick( {paisId: paisId, ciudadId: ciudadId }, logout )
        .then((res) => {
          setInfo(res)
          setLoading(false)
        })
